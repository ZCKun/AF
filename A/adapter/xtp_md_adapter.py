...
def start(config_path: str, queue: Queue, sub_symbol_codes: list[str]):
    try:
        ...
        if len(sub_symbol_codes) == 0:
            ...
            s = md.subscribe_market_data(sz_codes, len(sz_codes), XTP_EXCHANGE_TYPE.XTP_EXCHANGE_SZ)
            if s != 0:
                err = md.get_api_last_error()
                logger.error(f"XTP Subscribe SZ market data failed, error:{err}")
            else:
                logger.info("XTP Subscribe SZ market data success.")
            s = md.subscribe_market_data(sh_codes, len(sh_codes), XTP_EXCHANGE_TYPE.XTP_EXCHANGE_SH)
            if s != 0:
                err = md.get_api_last_error()
        ...
        while int(datetime.now().strftime("%H%M%S")) <= 150200:
            pass

    except Exception as e:
        logger.error(f"XTP Error: {e}")
